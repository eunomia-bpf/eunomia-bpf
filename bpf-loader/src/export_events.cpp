...
static bool is_string_type(const char* type_str) {
    return strncmp(type_str, "char[", 5) == 0;
}

static bool is_bool_type(const char* type_str) {
    return strncmp(type_str, "bool", 4) == 0;
}

static void btf_dump_event_printf(void* ctx, const char* fmt, va_list args) {
...
int event_exporter::check_export_types_btf(
    export_types_struct_meta& struct_meta) {
...
std::string event_exporter::get_plant_text_checked_types_header(
...
static int get_btf_type(unsigned int id,
...
int event_exporter::check_and_push_export_type_btf(
...
int event_exporter::check_sample_types_btf(
    unsigned int type_id,
    std::vector<checked_export_member>& checked_member,
...
int event_exporter::check_and_create_key_value_format(
...
int event_exporter::check_and_create_export_format(
    std::vector<export_types_struct_meta>& export_types,
    btf* btf_data) {
...
int event_exporter::sprintf_printer::vsprintf_event(const char* fmt,
                                                    va_list args) {
...
int event_exporter::sprintf_printer::snprintf_event(size_t __maxlen,
                                                    const char* fmt,
                                                    ...) {
    char output_buffer_pointer[EVENT_SIZE];
...
int event_exporter::sprintf_printer::sprintf_event(const char* fmt, ...) {
    va_list args;
    va_start(args, fmt);
...
void event_exporter::sprintf_printer::export_to_handler_or_print(
    void* user_ctx,
    export_event_handler& user_export_event_handler) {
...
void event_exporter::setup_btf_dumper(void) {
    assert(exported_btf);
    struct btf_dump* d =
...
int event_exporter::print_export_member(const char* event,
                                        std::size_t offset,
...
void event_exporter::dump_value_members_to_json(
    const char* event,
    std::vector<checked_export_member>& checker_members) {
...
void event_exporter::raw_event_handler(const char* event, size_t size) {
    printer.reset();
...
int event_exporter::print_sample_event_to_plant_text(
...
int event_exporter::print_sample_event_to_log2_hist(
    std::vector<char>& key_buffer,
    std::vector<char>& value_buffer) {
    printer.reset();
...
int event_exporter::raw_sample_handler(std::vector<char>& key_buffer,
                                       std::vector<char>& value_buffer) {
    printer.reset();
    if (user_export_event_handler) {
...
int event_exporter::print_sample_event_to_json(
    std::vector<char>& key_buffer,
    std::vector<char>& value_buffer) {
    printer.reset();
    printer.sprintf_event("{\"key\":");
...
void event_exporter::dump_value_members_to_plant_text(
    const char* event,
    std::vector<checked_export_member>& checker_members) {
...
void event_exporter::print_export_event_to_plant_text_with_time(
    const char* event,
    size_t size) {
    struct tm tm;
    char ts[32];
...
void event_exporter::handler_export_events(const char* event,
                                           size_t size) const {
    if (!event) {
        return;
    }
...
int event_exporter::handler_sample_key_value(
    std::vector<char>& key_buffer,
...
void event_exporter::set_export_type(export_format_type type,
...
void bpf_skeleton::handler_export_events(const char* event, size_t size) const {
    exporter.handler_export_events(event, size);
}

int bpf_skeleton::wait_and_poll_to_handler(enum export_format_type type,
                                           export_event_handler handler,
                                           void* ctx) noexcept {
