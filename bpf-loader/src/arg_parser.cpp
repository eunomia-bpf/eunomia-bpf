...
std::string get_default_var_help(data_section_variable_meta& var) {
    return std::string("set value of ") + var.type + " variable " + var.name;
}

std::string get_value_or_default(const json& j,
                                 const char* key,
...
void add_default_options(argparse::ArgumentParser& program,
...
int process_default_args(const argparse::ArgumentParser& program,
                         std::string& version,
                         json& json_config) {
...
argparse::ArgumentParser create_arg_parser_for_program(const json& j) {
...
void register_args_for_section_var(argparse::ArgumentParser& program,
                                   json& bpf_skel,
                                   std::vector<possible_option_arg>& opts) {
    bpf_skel_meta meta;
...
argparse::Argument& add_argument(argparse::ArgumentParser& program,
...
void add_argument_to_program(argparse::ArgumentParser& program,
                             const possible_option_arg& opt) {
...
void process_and_auto_create_arguments(std::vector<possible_option_arg>& opts) {
...
void set_program_arguments(argparse::ArgumentParser& program,
                           std::vector<possible_option_arg>& opts) {
...
template <typename T>
void process_arg_value(const argparse::ArgumentParser& program,
...
const std::map<
    std::string,
    std::function<void(const argparse::ArgumentParser&, possible_option_arg&)>>
...
int process_args_for_section_value(const argparse::ArgumentParser& program,
                                   json& bpf_skel,
                                   std::vector<possible_option_arg>& opts) {
    for (auto& opt : opts) {
        ...
        } else if ((argparse::details::starts_with(
...
int process_args(const argparse::ArgumentParser& program,
                 json& json_config,
                 std::vector<possible_option_arg>& opts,
                 std::string& version) {
...
void register_args(argparse::ArgumentParser& program,
                   json& bpf_skel,
...
int parse_args_for_json_config(const std::string& json_config,
