...
class event_exporter {
   ...
   private:
    ...
    using internal_sample_map_handler =
        std::function<int(std::vector<char>& key_buffer,
    ...
    std::unique_ptr<btf_dump, void (*)(btf_dump*)> btf_dumper{nullptr,
                                                              btf_dump__free};
    ...
    int print_export_member(const char* event,
                            std::size_t offset,
