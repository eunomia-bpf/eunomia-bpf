...
fn get_gh_env_token() -> (String, String) {
    ...
    (username, read_login_token())
}

async fn login_registry(url: Url, username: &str, token: &str) -> Result<()> {
    let Some(host) = url.host_str() else {
        return Err(Error::InvalidParam("url format incorrect".to_string()))
    };
...
pub async fn login(u: String) -> Result<()> {
    let url = Url::parse(u.as_str()).map_err(|e| Error::InvalidParam(e.to_string()))?;

    if url.as_str() == "https://ghcr.io/" {
        let (user, token) = get_gh_env_token();
