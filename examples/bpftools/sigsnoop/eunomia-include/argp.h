...
extern void (*argp_program_version_hook)(struct argp_state* __restrict __state);
...
extern void* __argp_input(
    __const struct argp* __restrict __argp,
    __const struct argp_state* __restrict __state) __THROW;
...
ARGP_EI void __argp_usage(__const struct argp_state* __state) {
    __argp_state_help(__state, stderr, ARGP_HELP_STD_USAGE);
}

ARGP_EI int __option_is_short(__const struct argp_option* __opt) {
...
ARGP_EI int __option_is_end(__const struct argp_option* __opt) {
...
static const struct argp_option* find_short_option(struct parser* parser,
                                                   int key,
                                                   struct group** p) {
    struct group* group;
...
static const struct argp_option* find_long_option(struct parser* parser,
                                                  const char* arg,
                                                  struct group** p) {
    struct group* group;
	   ...
	   just a few more times... */
        ...
        /* No way to return the remaining arguments, they must be bogus. */
        {
            if (!(parser->state.flags & ARGP_NO_ERRS))
                printf(dgettext(parser->argp->argp_domain,
...
static enum arg_type classify_arg(struct parser* parser,
                                  char* arg,
                                  char** opt) {
    if (arg[0] == '-')
    ...
    /* Deal with short options. */
    {
        ...
        if (!option) {
            if (parser->posixly_correct)
                ...
                printf(dgettext(parser->state.root_argp->argp_domain,
                                "%s: illegal option -- %c\n"),
                       parser->state.name, c);
            else
                printf(dgettext(parser->state.root_argp->argp_domain,
            ...
            switch (token) {
                    ...
                    switch (parser->ordering) {
                        case PERMUTE:
                            if (parser->first_nonopt == parser->last_nonopt)
                                ...
                                parser->first_nonopt = parser->last_nonopt =
                                    parser->state.next;
		     ...
		     if the user resets the next pointer. */
                ...
                case ARG_LONG_OPTION: {
                    ...
                    /* We need an mandatory argument. */
                    {
                        ...
                        /* Missing argument */
                        {
                            if (token == ARG_LONG_OPTION)
                                ...
                                printf(dgettext(
                                           parser->state.root_argp->argp_domain,
                                           "%s: option `%c%s' requires an "
                                           "argument\n"),
                                       parser->state.name, arg[0],
                                       option->name);
                    ...
                    *arg_ebadkey = 0;
                    return group_parse(group, &parser->state, option->key,
                                       value);
                }
...
int __option_is_short(__const struct argp_option* __opt) {
    if (__opt->flags & OPTION_DOC)
        return 0;
    else {
        int __key = __opt->key;
