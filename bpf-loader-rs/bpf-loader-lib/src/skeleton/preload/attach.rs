...
fn init_perf_monitor(freq: u64) -> Result<Vec<i32>> {
    ...
    for cpu in 0..nprocs {
        ...
        let pefd = unsafe {
            perf_event_open_sys::perf_event_open(&mut attrs, 0, cpu as i32, -1, 0)
        };
        if pefd < 0 {
...
pub(crate) fn attach_perf_event(program: &mut Program, _meta: &ProgMeta) -> Result<Vec<AttachLink>> {
    let pefds = init_perf_monitor(0).with_context(|| anyhow!("Failed to init perf monitor"))?;
    
    let mut links = vec![];
    for pefd in pefds.iter() {
        links.push(AttachLink::BpfLink(program.attach_perf_event(*pefd).unwrap()))
    }
    Ok(links)
}
