...
use btf::types::{Btf, BtfConst, BtfIntEncoding, BtfRestrict, BtfType, BtfTypedef, BtfVolatile};
use log::debug;
...
pub(crate) fn dump_to_json(btf: &Btf, type_id: u32, data: &[u8]) -> Result<Value> {
    ...
    match ty {
        ...
        BtfType::Struct(comp) | BtfType::Union(comp) => {
            ...
            for elem in comp.members.iter() {
                ...
                result.insert(
                    ...
                    dump_to_json(
                        btf,
                        elem.type_id,
                        &data[(elem.bit_offset / 8) as usize
                            ..(elem.bit_offset / 8 + elem_size) as usize],
                    )?,
                );
            }
            Ok(json!(result))
